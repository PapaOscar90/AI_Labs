...
void search(int mode, int start, int goal) {
    ...
    while (!isEmptyFringe(fringe)) {
        ...
        fringe = insertValidSucc(fringe, value + 1, state, 1, &visitedStates,
                                 &visitedStatesSize); /* rule n->n + 1      */
        fringe = insertValidSucc(fringe, 2 * value, state, 2, &visitedStates,
                                 &visitedStatesSize); /* rule n->2*n        */
        fringe = insertValidSucc(fringe, 3 * value, state, 2, &visitedStates,
                                 &visitedStatesSize); /* rule n->3*n        */
        fringe = insertValidSucc(fringe, value - 1, state, 1, &visitedStates,
                                 &visitedStatesSize); /* rule n->n - 1      */
        fringe = insertValidSucc(fringe, value / 2, state, 3, &visitedStates,
                                 &visitedStatesSize); /* rule n->floor(n/2) */
        fringe = insertValidSucc(fringe, value / 3, state, 3, &visitedStates,
                                 &visitedStatesSize); /* rule n->floor(n/3) */
    }
    if (goalReached == 0) {
...
void IDS(int mode, int start, int goal) {
    ...
    while (maxDepth <=
           IDS_DEPTH_LIMIT) { // takes around half a minute to reach this depth, after nearly a billion nodes visited
        state.value = start;
        state.prevValue = -1;
        state.cost = 0;
        state.length = -1;  //insertValidSucc increments this to 0 when it inserts this state
